...
def confusion_matrix_GT(n):
    ...
    grangers_causation_matrix(
        df_slope_POLITICIAN, variables=df_slope_POLITICIAN.columns[1:]
    )
...
def trend_p_value_GT():
    ts_df = pd.DataFrame(
        columns=["armonic_mean_normalized_Bots", "armonic_mean_Normalized_NoBots"],
        data=df_slope_POLITICIAN[
            ["armonic_mean_normalized_Bots", "armonic_mean_Normalized_NoBots"]
        ],
    )

    # Granger test
    granger_score = grangercausalitytests(
        ts_df[["armonic_mean_normalized_Bots", "armonic_mean_Normalized_NoBots"]],
        maxlag=9,
    )
    ...
    plt.bar(
        ...
        height=[granger_score[i + 1][0]["ssr_chi2test"][1] for i in range(9)],
        color="lightblue",
    )
