...
# Read config & Authenticate to Twitter

config = configparser.ConfigParser()
config.read('config.ini')

TWITTER_API_KEY = config['twitter']['TWITTER_API_KEY']
TWITTER_API_SECRET_KEY = config['twitter']['TWITTER_API_SECRET_KEY']
...
for party in parties:
    ...
    for user in party:
        ...
        with open(str(party) + '.csv', 'w', newline='') as csvfile:
            ...
            df = pd.read_csv(str(party) + '.csv')
            df.insert(len(response), 'weight', 1 if df['username'] == user[0] else 0.75 if df['username'] == user[1] else 0.25)
            df.to_csv(str(party) + '.csv')
